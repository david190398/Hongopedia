...
class Hongo{
    ...
    function aporte(){ //insertar hongo
    ...
    function aporte_csv(){
        ...
        $archivo_ruta= $this->sube_archivo($this->archivo_csv,$this->ruta);
        ...
        echo "<p> Tuplas insertadas correctamente: $fila <br /></p>\n";
        fclose($archivo); 

    }
    function genere_nombre_aleatorio($tamaño){
        $cadena="";
    ...
    function get_dato($id, $parametro_id , $nom_colum ,$nom_tabla){
        ...
        $sql = "SELECT {$nom_colum} from GestionHongos.dbo.{$nom_tabla} where {$parametro_id} = {$id}";
        $qry = sqlsrv_query($conn, $sql);
        if ($qry === false) {
        echo "Error (sqlsrv_query): " . print_r(sqlsrv_errors(), true);
        }
        $data = '';
        if($nom_colum != "fecha_avista"){
            ...
            while ($row = sqlsrv_fetch_array($qry, SQLSRV_FETCH_ASSOC)) {
                $data .= $row[$nom_colum]->format('Y-m-d');
            }
        }
    ...
    // METODO SET
    function set_dato($id, $parametro_id ,$nom_colum, $nom_tabla,$dato_nuevo){
        $serverName = "35.224.8.47"; //serverName\instanceName
        $connectionInfo = array("Database" => "master", "UID" => "sqlserver", "PWD" => "honguit0s");
        $conn = sqlsrv_connect($serverName, $connectionInfo);
        /*if ($conn) {
        ...
        $sql = "update GestionHongos.dbo.{$nom_tabla} set {$nom_colum} = '{$dato_nuevo}' where {$parametro_id} = {$id}";
        $qry = sqlsrv_query($conn, $sql);
        if ($qry === false) {
        echo "Error (sqlsrv_query): " . print_r(sqlsrv_errors(), true);
        }
    ...
    function delete_dato($id, $parametro_id,$nom_tabla) {
        $serverName = "35.224.8.47"; //serverName\instanceName
        $connectionInfo = array("Database" => "master", "UID" => "sqlserver", "PWD" => "honguit0s");
        $conn = sqlsrv_connect($serverName, $connectionInfo);
        /*if ($conn) {
        ...
        $sql = "delete from GestionHongos.dbo.{$nom_tabla} Where {$parametro_id} = {$id}";
        $qry = sqlsrv_query($conn, $sql);
        if ($qry === false) {
          echo "Error (sqlsrv_query): " . print_r(sqlsrv_errors(), true);
        }
     ...
     function sube_archivo($nombre_archivo, $rut){
        $aceptado=TRUE;
